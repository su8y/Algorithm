...
string solution(string m, vector<string> musicinfos) {
    ...
    for(int i=0;i<musicinfos.size();i++) {
        ...
        if (ContainsString(duration, scaledContent, target) && duration > durationMax) {
            answer = title;
...
int GetTime(string start, string end) {
    int startMin = stoi(start.substr(0,2)) * 60
        		 + stoi(start.substr(3,2));
    
    int endMin = stoi(end.substr(0,2)) * 60
        		 + stoi(end.substr(3,2));
